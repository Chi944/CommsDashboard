...
function normalizeEntry(entry, allowedOrigins, atom = false, allowExternalHttpsLinks = false) {
  const title = stripHtml(entry?.title);
  const rawUrl = linkValue(entry?.link, { preferAlternate: atom });
  // Google News is a trusted discovery feed, but may include canonical publisher
  // URLs rather than its redirector. Keep the legacy route contract while still
  // requiring HTTPS, stripping credentials/tracking, and never fetching item URLs.
  const url = canonicalizeSourceUrl(rawUrl, allowedOrigins)
    || (allowExternalHttpsLinks ? parseCanonicalUrl(rawUrl)?.toString() : null);
...
export function parseFeed(xml, {
  ...
  const items = rssItems.length
    ? rssItems.map((entry) => normalizeEntry(entry, allowedOrigins, false, allowExternalHttpsLinks))
    : atomEntries.map((entry) => normalizeEntry(entry, allowedOrigins, true, allowExternalHttpsLinks));
