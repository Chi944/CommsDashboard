import { readPrivateBlobForCas } from '../storage/blob-cas.js';
...
// Match the providers' twice-daily refresh window, including one hour of grace.
const REDIS_PREFERRED_MAX_AGE_MS = 13 * 60 * 60 * 1000;
...
async function writeToBlob(payload) {
  ...
  try {
    ...
    const headOptions = blobOptions({});
    const isNotFound = (error) => (
      error instanceof BlobNotFoundError || error?.name === 'BlobNotFoundError'
    );
    const writeOptions = {
      access: 'private',
    ...
    return await writeNewestBlobCache(payload, {
      read: async () => {
        const { result, etag } = await readPrivateBlobForCas({
        ...
        try {
          ...
          // Preserve the strong ETag so valid JSON can replace corrupt content.
        }
        return { data, etag };
      },
...
function redisCacheFreshness(data, nowMs) {
  if (!data) return null;
  const refreshedAtMs = Date.parse(data.refreshedAt);
  const versionMs = cacheVersionMs(data);
  if (versionMs === null || versionMs > refreshedAtMs || refreshedAtMs > nowMs) {
    return 'redis_invalid_cache';
  }
  return nowMs - versionMs > REDIS_PREFERRED_MAX_AGE_MS ? 'redis_stale_cache' : null;
}

/**
 * Default maintenance reads compare both durable stores. Public snapshots may
 * prefer a recent Redis mirror, avoiding an origin Blob GET on every CDN miss.
 * Provider observation/fetch freshness is still checked by each row reader.
 */
export async function readProviderCache(options = {}) {
  ...
  let blobProbe;
  let redisProbe;
  let blobReadSkipped = false;
  if (options.preferRedis === true) {
    redisProbe = await runReadProbe(options.readRedis || readFromRedis, 'redis', isRedisConfigured);
    const freshnessError = redisCacheFreshness(redisProbe.data, nowMs);
    if (freshnessError) {
      redisProbe = {
        // Expired data remains eligible for a visibly stale last-good fallback;
        // invalid/future versions must not outrank a valid origin payload.
        data: freshnessError === 'redis_stale_cache' ? redisProbe.data : null,
        error: redisProbe.error || freshnessError,
      };
    }
    if (redisProbe.data && !redisProbe.error) {
      blobReadSkipped = isBlobConfigured;
      blobProbe = { data: null, error: null };
    } else {
      blobProbe = await runReadProbe(options.readBlob || readFromBlob, 'blob', isBlobConfigured);
    }
  } else {
    [blobProbe, redisProbe] = await Promise.all([
