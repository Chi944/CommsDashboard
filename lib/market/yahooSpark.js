...
export const YAHOO_SPARK_RECOVERY_REQUEST_LIMIT = 12;
...
async function fetchYahooSparkBatch(batch, fetchImpl) {
...
function canIsolate(result) {
  return result.error && result.error !== 'timeout' && result.batch.length > 1;
}
...
export async function fetchYahooSparkBatches(symbols, {
  ...
  recoveryRequestLimit = YAHOO_SPARK_RECOVERY_REQUEST_LIMIT,
} = {}) {
  const batches = batchesOf(symbols, batchSize);
  let requestCount = 0;
  let recoveryBudget = Math.max(0, Math.floor(Number(recoveryRequestLimit) || 0));
  const requestBatch = async (batch) => {
    requestCount += 1;
    return fetchYahooSparkBatch(batch, fetchImpl);
  };
  const settled = await Promise.all(batches.map(requestBatch));

  const completed = settled.filter((result) => !result.error);
  const finalFailures = settled.filter((result) => result.error && !canIsolate(result));
  const retryable = settled.filter(canIsolate);

  const retryCount = Math.min(retryable.length, recoveryBudget);
  recoveryBudget -= retryCount;
  const retried = await Promise.all(retryable.slice(0, retryCount).map((result) => requestBatch(result.batch)));
  completed.push(...retried.filter((result) => !result.error));
  finalFailures.push(...retryable.slice(retryCount));

  let isolationQueue = retried.filter(canIsolate);
  finalFailures.push(...retried.filter((result) => result.error && !canIsolate(result)));

  while (isolationQueue.length > 0 && recoveryBudget >= 2) {
    const parentsThisRound = isolationQueue.slice(0, Math.floor(recoveryBudget / 2));
    isolationQueue = isolationQueue.slice(parentsThisRound.length);
    finalFailures.push(...isolationQueue);

    const children = parentsThisRound.flatMap((result) => splitBatch(result.batch));
    recoveryBudget -= children.length;
    const childResults = await Promise.all(children.map(requestBatch));

    completed.push(...childResults.filter((result) => !result.error));
    finalFailures.push(...childResults.filter((result) => result.error && !canIsolate(result)));
    isolationQueue = childResults.filter(canIsolate);
  }
  finalFailures.push(...isolationQueue);
  ...
  return { bySymbol, errors, requestCount };
}
