...
const configuredIds = [...new Set(Object.values(COINGECKO_IDS))];
...
export async function fetchCoinGeckoVolumes() {
  ...
  url.searchParams.set('ids', configuredIds.join(','));
  url.searchParams.set('order', 'volume_desc');
  url.searchParams.set('per_page', String(configuredIds.length));
  ...
  try {
    ...
    return { volumes: {}, errors: [`coingecko markets ${providerFailureCode(error)}`] };
  }
  if (!r.ok) {
  ...
  try {
    ...
    return { volumes: {}, errors: ['coingecko markets invalid_response'] };
  }
  const volumes = {};
  const errors = [];
