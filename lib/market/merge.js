...
export function mergeMarketSnapshot(liveRows, fallbackRows = [], options = {}) {
  ...
  const commodities = SYMBOLS.filter((s) => s.category !== 'FX').map((s) => {
    ...
    if (!live || typeof live.source !== 'string' || !live.source.trim()) return null;
    return {
      ...(fb || s),
      ...live,
  ...
  const sources = {
    coingecko: commodities.filter((r) => r.source === 'coingecko').length,
    alphavantage: commodities.filter((r) => r.source === 'alphavantage').length,
    eia: commodities.filter((r) => r.source === 'eia').length,
  };
  ...
  const coveredRows = commodities.filter((row) => liveTickers.includes(row.ticker));
  const covered = coveredRows.length;
  const live = coveredRows.filter((row) => row.stale === false).length;
  const stale = covered - live;
  const expected = liveTickers.length;
  const counts = {
    expected,
    returned: commodities.length,
    covered,
    live,
    stale,
    fallback: 0,
  ...
  return {
    ...
    meta: {
      ...
      coverage: {
        covered,
        expected,
        ratio: expected ? Number((covered / expected).toFixed(2)) : 1,
      },
    },
  };
}
