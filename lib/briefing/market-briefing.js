...
export function validateMarketBriefingCompletion(completion, context, options = {}) {
  ...
  const hasGainerEvidence = evidence.some((record) => (
    record.type === 'top_gainer' && observedChange(record) > 0
  ));
  const hasLoserEvidence = evidence.some((record) => (
    record.type === 'top_loser' && observedChange(record) < 0
  ));
  const hasHeadlineEvidence = evidence.some((record) => record.type === 'headline');
  const hasSentimentEvidence = evidence.some((record) => (
