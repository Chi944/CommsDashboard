...
function storageConfiguration(diagnostics) {
  return diagnostics?.configurationError == null
    && diagnostics?.blob === true
    && diagnostics?.redis === true
    && diagnostics?.blobError == null
    && diagnostics?.redisError == null
    ? 'ready'
    : 'unavailable';
}
...
export function buildSmartMoneyHealth(input = {}) {
  ...
  if (input.snapshot !== null && input.snapshot !== undefined) {
    const accepted = validateSmartMoneyPrivateSnapshot(input.snapshot, { now });
    rawStatuses = accepted.publicSnapshot.providerStatuses.map((status) => normalizeProviderStatus(
      status,
  ...
  const storage = storageConfiguration(input.storageDiagnostics);
  ...
  return {
    schemaVersion: 1,
    ok: rightsConfigured && storage === 'ready' && secUserAgent === 'configured' && !degradedProviders,
