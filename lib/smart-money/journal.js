  ...
  try {
    ...
    return {
      async readData(pathname) {
        ...
        // No conditional request or cached body exists on this path. A 304,
        // missing stream, or invalid body must not masquerade as an absent file.
        if (!result?.stream) throw journalError('journal_read_failed');
        const data = await blobData(result);
        if (data === null) throw journalError('journal_read_failed');
        return { data };
      },
      async read(pathname) {
        ...
        return { data: await blobData(result), etag };
      },
...
async function resolveReadOnlyAdapter(options) {
  const adapter = await resolveAdapter(options);
  // Preserve injected adapters. Production data-only reads intentionally expose
  // neither a write method nor an ETag that could be reused for a mutation.
  if (!adapter?.readData) return adapter;
  return { read: (pathname) => adapter.readData(pathname) };
}
...
export async function readAcceptedSmartMoneySnapshot(options = {}) {
  const adapter = await resolveReadOnlyAdapter(options);
