...
const STAGED_RETRY_GRACE_DAYS = 7;
...
function canonicalJsonValue(value) {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw schemaInvalid();
    return value;
  }
  ...
  for (const [key, child] of Object.entries(value)) {
    if (['__proto__', 'prototype', 'constructor'].includes(key)) throw schemaInvalid();
    result[key] = canonicalJsonValue(child);
  }
  return result;
}

function normalizeAcceptedSnapshot(value, generation, digest) {
  assertSafePlainRecord(value, [
    'schemaVersion', 'refreshStartedAt', 'publicSnapshot', 'adapterState', 'stateDigest',
  ]);
  if (value.schemaVersion !== 1 || canonicalInstant(value.refreshStartedAt) !== generation
      || typeof value.stateDigest !== 'string' || !DIGEST_PATTERN.test(value.stateDigest)
      || value.stateDigest !== digest) throw schemaInvalid();
  const snapshot = canonicalJsonValue(value);
  assertSafePlainDataRecord(snapshot.publicSnapshot);
  assertSafeArray(snapshot.publicSnapshot.signals);
  const signalIds = snapshot.publicSnapshot.signals.map((signal) => {
    assertSafePlainDataRecord(signal);
...
function normalizeCurrentPublication(value, published) {
  if (value === null) {
  ...
  const { snapshot, signalIds } = normalizeAcceptedSnapshot(
    value.snapshot, refreshStartedAt, value.snapshotDigest,
  );
...
function normalizePublications(value) {
  ...
  return {
    ...
    published,
    current: normalizeCurrentPublication(value.current, published),
  };
}
...
async function casPublications(adapter, update) {
  for (let attempt = 0; attempt < CAS_ATTEMPTS; attempt += 1) {
    let current;
    let publications;
    try {
      current = await adapter.read(JOURNAL_PUBLICATIONS);
      publications = normalizePublications(current?.data);
    } catch {
    ...
    const next = normalizePublications(updated.data);
    try {
      ...
      return {
        ...
        skipped: false,
        error: null,
        data: normalizePublications(committed?.data),
      };
    } catch (error) {
      try {
        const committed = await adapter.read(JOURNAL_PUBLICATIONS);
        const reread = normalizePublications(committed?.data);
...
export async function publishJournalGeneration(input, options = {}) {
  ...
  const accepted = normalizeAcceptedSnapshot(
    input.snapshot, refreshStartedAt, input.snapshot.stateDigest,
  );
  let adapter;
  try {
    adapter = await resolveAdapter(options);
...
export async function readAcceptedSmartMoneySnapshot(options = {}) {
  ...
  const publications = normalizePublications(record?.data);
...
async function readRetainedJournal(adapter, now) {
  ...
  const publications = normalizePublications(publicationRecord?.data);
  ...
  for (const date of manifest.partitions) {
    ...
    for (const signal of partition.signals) {
      if (manifest.signalIds[signal.id] !== date) continue;
    ...
    for (const mark of partition.dailyMarks) {
      if (manifest.dailyMarkIds[mark.id] !== date) continue;
...
async function prunePublicationMetadata(adapter, manifest, now) {
  ...
  const retryCutoff = new Date(now.getTime() - STAGED_RETRY_GRACE_DAYS * DAY_MS).toISOString();
  return casPublications(adapter, (current) => {
    const staged = Object.fromEntries(Object.entries(current.staged).filter(([generation]) => (
      generation >= retryCutoff
    )));
    const published = {};
...
export async function pruneJournal(input, options = {}) {
  ...
  const publicationsResult = await prunePublicationMetadata(adapter, manifestResult.data, now);
  ...
  const cleanupDates = new Set(expired);
...
export const SMART_MONEY_JOURNAL_NAMESPACES = Object.freeze({
  ...
  stagedRetryGraceDays: STAGED_RETRY_GRACE_DAYS,
});
