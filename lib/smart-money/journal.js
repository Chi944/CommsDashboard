...
function emptyPublications() {
  return { schemaVersion: 1, staged: {}, published: {} };
}
...
function normalizePublications(value) {
  ...
  assertSafePlainRecord(value, ['schemaVersion', 'staged', 'published']);
  if (value.schemaVersion !== 1) throw schemaInvalid();
  return {
    schemaVersion: 1,
    staged: normalizeGenerationIndex(value.staged),
    published: normalizeGenerationIndex(value.published),
  };
}
...
export async function publishJournalGeneration(input, options = {}) {
  assertSafePlainRecord(input, ['refreshStartedAt']);
  const now = normalizedNow(options.now ?? new Date());
  const refreshStartedAt = canonicalInstant(input.refreshStartedAt);
  if (Date.parse(refreshStartedAt) > now.getTime() + 5 * 60_000) throw schemaInvalid();
  let adapter;
  ...
  const published = await casPublications(adapter, (current) => {
    ...
    if (existing) {
      if (staged && !samePublicationIds(existing, staged)) throw schemaInvalid();
      return { skipped: true, data: current };
    }
    if (!staged) throw schemaInvalid();
    return {
      skipped: false,
      data: {
        ...current,
        published: { ...current.published, [refreshStartedAt]: staged },
      },
    };
  });
  return {
