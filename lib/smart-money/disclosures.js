...
import { fetchSecInstitutionalDisclosure } from './sec.js';
...
function nullableNonnegativeNumber(value, providerId) {
  return value === null ? null : nonnegativeNumber(value, providerId);
}
...
function normalizeDisclosure(raw, givenConfig, deps = {}) {
  ...
  const rawReportedValue = Object.hasOwn(raw, 'reportedValueUsd')
    ? raw.reportedValueUsd
    : raw.valueUsd;
  const reportedValueUsd = nullableNonnegativeNumber(rawReportedValue, config.id);
  if (reportedValueUsd === null && config.id !== 'institutional-tesla') throw schemaError(config.id);
...
function rawFetcher(deps, config) {
  ...
  if (typeof fetchRaw === 'function') return fetchRaw(config, deps);
  return fetchSecInstitutionalDisclosure({
    providerId: config.id,
    cik: config.cik,
...
function validateNormalizedRecord(record, config, envelopeRetrievedAt) {
  ...
  nullableNonnegativeNumber(record.reportedValueUsd, config.id);
  if (record.reportedValueUsd === null && config.id !== 'institutional-tesla') throw schemaError(config.id);
