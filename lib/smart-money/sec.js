...
const SCHEDULE_FORMS = new Set(['SC 13D', 'SC 13D/A', 'SC 13G', 'SC 13G/A']);
...
function secPrimaryDocument(value) {
  if (typeof value !== 'string' || value.length < 1 || value.length > 512
      || value.startsWith('/') || value.endsWith('/')) return null;
  const segments = value.split('/');
  if (segments.length > 5 || segments.some((segment) => (
    !/^[A-Za-z0-9._-]{1,255}$/.test(segment) || segment === '.' || segment === '..'
  ))) return null;
  return value;
}

function canonicalSecForm(value) {
  const form = textValue(value).toUpperCase().replace(/\s+/g, ' ');
  const schedule = form.match(/^(?:SC|SCHEDULE) 13([DG])(\/A)?$/);
  return schedule ? `SC 13${schedule[1]}${schedule[2] || ''}` : form;
}
...
function informationTableName(index) {
  ...
  const candidates = names.filter((name) => /(?:information|info)[._-]?table/i.test(name));
  if (candidates.length !== 1) throw schemaError();
  return candidates[0];
}

function parseSecArchiveIndex(value) {
  try {
    const parsed = JSON.parse(value);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw schemaError();
    return parsed;
  } catch (error) {
...
export function parseSecSubmissions(payload, config = {}) {
  ...
  for (let index = 0; index < forms.length; index += 1) {
    const form = canonicalSecForm(forms[index]);
    const thirteenF = THIRTEEN_F_FORMS.has(form);
    const schedule = SCHEDULE_FORMS.has(form);
    if (!thirteenF && !schedule) continue;
    const accessionNumber = textValue(recent.accessionNumber?.[index]);
    const reportDate = textValue(recent.reportDate[index]);
    const filedAt = isoFilingDate(recent.filingDate?.[index]);
    const primaryDocument = secPrimaryDocument(textValue(recent.primaryDocument?.[index]));
    if (!/^\d{10}-\d{2}-\d{6}$/.test(accessionNumber) || !filedAt
        || primaryDocument === null
        || (thirteenF && !isoFilingDate(reportDate))) {
      throw schemaError();
    }
    const filing = {
      cik: expectedCik,
      form,
      accessionNumber,
      periodEnd: thirteenF ? reportDate : null,
      filedAt,
      primaryDocument,
      isAmendment: /\/A$/.test(form),
    };
    if (schedule) {
      filing.amendmentChain = [accessionNumber];
      filing.timingBasis = 'filing_date';
    }
    filings.push(filing);
  }
  return filings;
...
export async function fetchSecSnapshot(config = {}, deps = {}) {
  ...
  const archiveIndexOptions = {
    ...archiveOptions,
    acceptedContentTypes: ['application/json', 'text/json', 'text/html'],
    maxBytes: 1_000_000,
    requestOptions: {
      headers: { ...headers, Accept: 'application/json, text/json, text/html' },
    },
  };
  ...
  if (filings.length === 0) throw new ProviderError('schema_invalid', providerId);
  const holdings = [];
  for (const filing of filings) {
    const index = parseSecArchiveIndex(await getText(
      filingIndexUrl(cik, filing.accessionNumber), archiveIndexOptions,
    ));
  ...
  if (holdings.length === 0) throw new ProviderError('schema_invalid', providerId);
  return {
    filings,
