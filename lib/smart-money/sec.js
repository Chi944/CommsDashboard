...
function factValues($, profile, providerId, concept, measure, scale, decimals, expectedReportDate) {
...
function institutionalRecentTuple(payload, profile, providerId) {
  ...
  const latestIndex = recent.form.findIndex((form) => ['10-Q', '10-K'].includes(form));
  if (latestIndex < 0) throw institutionalError(providerId);
  const latest = {
