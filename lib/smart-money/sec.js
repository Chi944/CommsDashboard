...
const INSTITUTIONAL_FILING_PROFILES = Object.freeze({
  ...
  'institutional-tesla': Object.freeze({
    cik: '1318605',
    quantityConcept: 'us-gaap:CryptoAssetNumberOfUnits', quantityMeasure: 'tsla:Bitcoin',
    quantityScale: 0, valueConcept: null, valueMeasure: null, valueScale: null,
    anchors: ['Digital assets, net', 'Bitcoin'], quantityAnchors: ['Digital assets, net', 'Bitcoin'], dimension: null,
  }),
  ...
  'institutional-fbtc': Object.freeze({
    ...
    dimension: Object.freeze({
      axis: 'us-gaap:InvestmentIdentifierAxis',
  ...
  'institutional-arkb': Object.freeze({
    ...
    anchors: ['Schedule of Investments', 'Bitcoin', 'Fair Value'],
    quantityAnchors: ['Bitcoin'], valueAnchors: ['Bitcoin', 'Fair Value'], dimension: null,
  }),
...
function exactContext($, contextId, profile, providerId, expectedReportDate = null) {
  ...
  if (contexts.length !== 1) throw institutionalError(providerId);
  ...
  if (identifiers.length !== 1 || instants.length !== 1
      ...
      || (expectedReportDate !== null && instant !== expectedReportDate)) {
    throw institutionalError(providerId);
  }
  ...
  if (profile.dimension === null) {
    if (explicitMembers.length !== 0 || typedMembers.length !== 0) throw institutionalError(providerId);
  } else if (explicitMembers.length !== 0 || typedMembers.length !== 1
...
function factValues($, profile, providerId, concept, measure, scale, decimals, expectedReportDate, anchors) {
  const facts = $('*').filter((_index, element) => (
    String(element.tagName || element.name || '').toLowerCase() === 'ix:nonfraction'
    && $(element).attr('name') === concept
    && anchors.every((anchor) => normalizedDocumentText($(element).closest('table').text()).includes(anchor))
  ));
  ...
  facts.each((_index, element) => {
    const fact = $(element);
    const tableText = normalizedDocumentText(fact.closest('table').text());
    if (!anchors.every((anchor) => tableText.includes(anchor))
        || fact.attr('xsi:nil') === 'true' || fact.attr('nil') === 'true'
        || fact.attr('scale') !== String(scale)
        || fact.attr('sign') != null
        || (decimals != null && fact.attr('decimals') !== String(decimals))) {
      throw institutionalError(providerId);
    }
    reportDates.push(exactContext(
      $, fact.attr('contextref'), profile, providerId, expectedReportDate,
    ));
    exactUnitMeasure($, fact.attr('unitref'), measure, providerId);
    values.push(exactInlineNumber(fact.text(), scale, providerId));
  });
  if (new Set(values.map(String)).size !== 1 || new Set(reportDates).size !== 1) {
    throw institutionalError(providerId);
  }
  return { value: values[0], reportDate: reportDates[0] };
}
...
export function parseSecInstitutionalDisclosureDocument(document, providerId, options = {}) {
  ...
  if (!profile.anchors.every((anchor) => documentText.includes(anchor))) {
    throw institutionalError(providerId);
  }
  const quantity = factValues(
    $, profile, providerId, profile.quantityConcept, profile.quantityMeasure,
    profile.quantityScale, profile.quantityDecimals, options.reportDate ?? null,
    profile.quantityAnchors,
  );
  const value = profile.valueConcept === null
    ...
    : factValues(
      ...
      profile.valueScale, null, options.reportDate ?? quantity.reportDate,
      profile.valueAnchors,
    );
...
function institutionalRecentTuple(payload, profile, providerId) {
  ...
  const candidates = Array.from({ length }, (_unused, index) => ({
    accessionNumber: recent.accessionNumber[index],
    filingDate: recent.filingDate[index],
    reportingDate: recent.reportDate[index],
    form: recent.form[index],
    primaryDocument: recent.primaryDocument[index],
  })).filter((filing) => ['10-Q', '10-K'].includes(filing.form)
    && /^\d{10}-\d{2}-\d{6}$/.test(filing.accessionNumber)
    && isoFilingDate(filing.filingDate) !== null
    && isoFilingDate(filing.reportingDate) !== null
    && /^[A-Za-z0-9._-]+\.html?$/i.test(filing.primaryDocument));
  const latest = candidates.sort((left, right) => (
    right.reportingDate.localeCompare(left.reportingDate)
    || right.filingDate.localeCompare(left.filingDate)
    || right.accessionNumber.localeCompare(left.accessionNumber)
  ))[0];
  if (!latest) throw institutionalError(providerId);
  return latest;
}
...
export async function fetchSecInstitutionalDisclosure(config = {}, deps = {}) {
  ...
  const archiveOptions = {
    ...transportOptions(providerId, [SEC_ARCHIVES_ORIGIN], headers, scheduler),
    maxBytes: 5_000_000,
  };
