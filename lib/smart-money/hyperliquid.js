...
function assembleMaintenanceSnapshot(leaderboard, detailRequests, detailResults) {
  const accounts = [];
  const portfolios = [];
  for (let index = 0; index < detailResults.length; index += 1) {
    const envelope = {
      providerId: 'hyperliquid-info-api', records: detailResults[index], linkOnly: false,
    };
    if (detailRequests[index]?.type === 'clearinghouseState') accounts.push(envelope);
    else if (detailRequests[index]?.type === 'portfolio') portfolios.push(envelope);
  }
  return {
    providerId: 'hyperliquid-leaderboard',
    performances: Array.isArray(leaderboard?.records) ? leaderboard.records : [],
    accounts,
    portfolios,
    fills: [],
    linkOnly: false,
...
async function fetchHyperliquidMaintenanceSnapshot(config, deps, capability) {
  ...
  return assembleMaintenanceSnapshot(leaderboard, plan.detailRequests, detailResults);
}
...
export function createHyperliquidProtectedMaintenanceAdapter() {
  return Object.freeze({
    ...
    assembleSnapshot: assembleMaintenanceSnapshot,
  });
}
