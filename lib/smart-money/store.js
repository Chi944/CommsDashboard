...
import { computeSmartMoneyPrivateStateDigest } from './private-snapshot.js';
...
const STATE_DIGEST_PATTERN = /^sha256:[0-9a-f]{64}$/;
  ...
  try {
    ...
    return await writeNewestSmartMoneyBlob(payload, {
      read: async () => {
        ...
        return { data: await blobJson(result), etag };
      },
...
function diagnosticFingerprint(data) {
  if (data === null || data === undefined) {
    return { hit: false, generation: null, digest: null };
  }
  let digest = null;
  try {
    if (typeof data.stateDigest === 'string'
        && STATE_DIGEST_PATTERN.test(data.stateDigest)
        && computeSmartMoneyPrivateStateDigest(data) === data.stateDigest) {
      digest = data.stateDigest;
    }
  } catch {
    // A stored payload is a hit, but it has no trustworthy fingerprint.
  }
  return {
    hit: true,
    generation: digest !== null && generationMs(data) !== null ? data.refreshStartedAt : null,
    digest,
  };
}
...
export async function readSmartMoneySnapshot(options = {}) {
  ...
  const blobFingerprint = diagnosticFingerprint(blob.data);
  const redisFingerprint = diagnosticFingerprint(redis.data);
  const diagnostics = {
    configurationError: null,
    blob: configuration.blobConfigured,
    blobHit: blobFingerprint.hit,
    blobGeneration: blobFingerprint.generation,
    blobDigest: blobFingerprint.digest,
    redis: configuration.redisConfigured,
    redisHit: redisFingerprint.hit,
    redisGeneration: redisFingerprint.generation,
    redisDigest: redisFingerprint.digest,
