...
if currentPayload and not currentVersion then
  ...
  if decodedOk and type(decoded) == 'table' then
    local generation = decoded.refreshStartedAt
    local canonical = string.match(
      generation or '',
      '^%d%d%d%d%-%d%d%-%d%dT%d%d:%d%d:%d%d%.%d%d%dZ$'
    )
    if canonical and generation >= ARGV[3] then
      return 0
    end
  end
end
...
export async function writeNewestSmartMoneyRedis(payload, redis) {
  ...
  try {
    const result = await redis.eval(
      ...
      [String(incomingVersion), JSON.stringify(payload), new Date(incomingVersion).toISOString()],
    );
...
async function writeProbe(writer, source, configured, payload) {
  ...
  try {
    ...
    return result?.ok
      ? { configured: true, ok: true, error: null, ...(result.skipped ? { skipped: true } : {}) }
      : { configured: true, ok: false, error: `${source}_write_failed` };
  } catch {
...
export async function writeSmartMoneySnapshot(snapshot, options = {}) {
  ...
  if (configuration.invalid || invalidSnapshot) {
    return {
      snapshot,
      blobWrite: { configured: configuration.blobConfigured, ok: false, error: configuration.blobConfigured ? 'blob_write_failed' : null },
      redisWrite: { configured: configuration.redisConfigured, ok: false, error: configuration.redisConfigured ? 'redis_write_failed' : null },
