...
export async function readDurableSmartMoneyCandidate(options = {}) {
  ...
  if (configuration.invalid) return null;
  const configured = [];
  ...
  if (configured.length === 0 || configured.some((probe) => probe.error || probe.data === null)) {
    return null;
  }
  const canonical = stableJson(configured[0].data);
  if (configured.some((probe) => stableJson(probe.data) !== canonical)) return null;
  return structuredClone(configured[0].data);
}
