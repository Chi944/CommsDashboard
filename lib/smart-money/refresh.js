...
import { listTrackedTickers, publishJournalGeneration, stageJournal } from './journal.js';
...
export function validateSmartMoneyPrivateSnapshot(value, options = {}) {
  ...
  const stateDigest = privateStateDigest(publicSnapshot, adapterState);
...
function privateStateDigest(publicSnapshot, adapterState) {
  return `sha256:${createHash('sha256').update(stableJson({ publicSnapshot, adapterState })).digest('hex')}`;
}
...
export function buildSmartMoneyPrivateSnapshot(value, options = {}) {
  ...
  return {
    ...
    publicSnapshot,
    adapterState,
    stateDigest: privateStateDigest(publicSnapshot, adapterState),
  };
}
