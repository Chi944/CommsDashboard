...
import { appendJournal, listTrackedTickers } from './journal.js';
...
import { readSmartMoneySnapshot, writeSmartMoneySnapshot } from './store.js';
...
function publicStatus({ id, status, now, previous = null, recordCount = 0, errorCode = null }) {
  ...
  for (const adapter of adapters) {
    ...
    if (dueIds.has(adapter.id) && result?.status === 'fulfilled') {
      if (adapter.id === 'sec-edgar') {
        source = { kind: 'sec', snapshot: structuredClone(result.value) };
        const count = result.value.filings.length + result.value.disclosures.length;
        status = publicStatus({
          id: adapter.id, status: count > 0 ? 'live' : 'unavailable', now,
          previous: prior?.status, recordCount: count,
          errorCode: count > 0 ? null : 'empty_dataset',
        });
        if (count > 0) changes.push(...buildSecHoldingChanges(source, prior?.source, now));
      } else {
        source = { kind: 'institutional', records: structuredClone(result.value.records) };
        ...
        if (count > 0) changes.push(...institutionalHoldingChanges(adapter.id, source, prior?.source, now));
      }
...
export function createProductionSmartMoneyDependencies(nowValue = new Date()) {
  ...
  return {
    ...
    withRefreshLock,
    assertAdapterRights,
    readSnapshot: readSmartMoneySnapshot,
    ...
    appendJournal: (input) => appendJournal(input, { now: exactNow(completionNow()) }),
...
export function createSmartMoneyRefresher(givenDependencies = null) {
  return async function refreshSmartMoney(_input = {}) {
    ...
    return lock(async () => {
      ...
      const journal = await deps.appendJournal({ signals: pricedSignals, dailyMarks });
      ...
      validateSmartMoneyPrivateSnapshot(privateSnapshot, { now: publicationNow });
