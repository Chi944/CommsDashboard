...
const FORM_13F_DOLLAR_REPORTING_START = '2023-01-03';
...
function form13fReportedValueUsd(reportedValue, filedAt) {
  if (reportedValue == null) return 0;
  const filingDate = String(filedAt).slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(filingDate)) throw schemaInvalid();
  // EDGAR Release 22.4.1 changed Form 13F values from thousands to dollars.
  const scale = filingDate >= FORM_13F_DOLLAR_REPORTING_START ? 1 : 1_000;
  const dollars = reportedValue * scale;
  if (!Number.isSafeInteger(dollars) || dollars < 0) throw schemaInvalid();
  return dollars;
}
...
export function buildSecHoldingChanges(currentSource, previousSource, nowValue) {
  ...
  return compare13FPeriods(current, previous).map((change) => {
    ...
    const reportedValueUsd = form13fReportedValueUsd(
...
function logJournalPersistenceFailure(journal) {
  const safeCode = (value, fallback) => (
    typeof value === 'string' && /^[a-z0-9][a-z0-9_]{0,63}$/.test(value)
      ? value
      : fallback
  );
  const partitions = Array.isArray(journal?.partitions)
    ? journal.partitions.slice(0, 32).map((partition) => ({
      ok: partition?.ok === true,
      errorCode: partition?.ok === true
        ? null
        : safeCode(partition?.error, 'partition_write_failed'),
    }))
    : [];
  console.error('[smart-money]', JSON.stringify({
    event: 'journal_persistence_failed',
    manifest: {
      ok: journal?.manifest?.ok === true,
      errorCode: journal?.manifest?.ok === true
        ? null
        : safeCode(journal?.manifest?.error, 'manifest_write_failed'),
    },
    partitions,
  }));
}
...
function candidateAbandonmentEvidence(candidateStatus, accepted) {
  return {
    candidateStatus,
    current: accepted === null ? null : {
      refreshStartedAt: accepted.refreshStartedAt,
      snapshotDigest: accepted.stateDigest,
    },
  };
}

async function abandonJournalStages(deps, now, abandonment) {
  if (typeof deps.pruneJournal !== 'function') {
    return { ok: true, pruneOk: true, skipped: true };
  }
  try {
    const result = await deps.pruneJournal({ now: now.toISOString(), abandonment });
    return {
      ok: result?.abandonment?.ok === true,
      pruneOk: result?.durableWriteSucceeded === true,
      pending: result?.abandonment?.pending === true,
    };
  } catch {
    return { ok: false, pruneOk: false, pending: false };
  }
}
...
export function createSmartMoneyRefresher(givenDependencies = null) {
  return async function refreshSmartMoney(_input = {}) {
    ...
    return lock(async () => {
      ...
      let startupAbandonmentEvidence = null;
      let startupPruneFailed = false;
      if (typeof deps.readCandidateSnapshot === 'function') {
        ...
        if (candidateResult?.status === 'ready') {
          ...
          let recoveryPreparation = { ok: true };
          let recovered = await deps.publishJournalGeneration({
            refreshStartedAt: candidate.refreshStartedAt,
            snapshot: candidate,
          });
          if (recovered.durableWriteSucceeded !== true) {
            recoveryPreparation = await pruneAfterPublication(
              deps, exactNow(completionNow()),
            );
            recovered = await deps.publishJournalGeneration({
              refreshStartedAt: candidate.refreshStartedAt,
              snapshot: candidate,
            });
          }
          ...
          if (stored === null || candidate.refreshStartedAt > previous.refreshStartedAt) {
            ...
            return {
              ...
              partial: candidate.publicSnapshot.partial
                || !recoveryPreparation.ok || !recoveryPrune.ok,
              ...
              warnings: [
                ...
                ...(recoveryPreparation.ok && recoveryPrune.ok
                  ? [] : ['journal:prune_failed']),
              ],
              errorCode: null,
            };
          }
          startupAbandonmentEvidence = candidateAbandonmentEvidence('ready', candidate);
        } else if (candidateResult?.status !== 'absent') {
          return candidateFailure(previous, 'candidate_storage_unavailable');
        } else {
          startupAbandonmentEvidence = candidateAbandonmentEvidence('absent', stored);
        }
      }
      if (startupAbandonmentEvidence && typeof deps.pruneJournal === 'function') {
        const reconciled = await abandonJournalStages(deps, exactNow(completionNow()), {
          mode: 'expired',
          through: invocationNow.toISOString(),
          evidence: startupAbandonmentEvidence,
        });
        if (!reconciled.ok) {
          return candidateFailure(
            previous,
            reconciled.pending ? 'journal_reconciliation_pending' : 'journal_reconciliation_failed',
          );
        }
        startupPruneFailed = !reconciled.pruneOk;
      }
      const previousById = new Map(previous.adapterState.adapters.map((row) => [row.id, row]));
      ...
      if (journal.durableWriteSucceeded !== true) {
        logJournalPersistenceFailure(journal);
      ...
      if (written.durableWriteSucceeded !== true) {
        if (typeof deps.readCandidateSnapshot === 'function') {
          let afterFailure;
          try {
            afterFailure = await deps.readCandidateSnapshot();
          } catch {
            afterFailure = { status: 'unavailable' };
          }
          if (afterFailure?.status === 'ready') {
            let candidate = null;
            try {
              assertSafePlainRecord(afterFailure, ['status', 'snapshot']);
              candidate = validateSmartMoneyPrivateSnapshot(afterFailure.snapshot, {
                now: exactNow(completionNow()),
              });
            } catch {
              candidate = null;
            }
            if (candidate?.refreshStartedAt === privateSnapshot.refreshStartedAt
                && candidate.stateDigest === privateSnapshot.stateDigest) {
              const recovered = await deps.publishJournalGeneration({
                refreshStartedAt: candidate.refreshStartedAt,
                snapshot: candidate,
              });
              if (recovered.durableWriteSucceeded === true) {
                const recoveredPrune = await pruneAfterPublication(deps, exactNow(completionNow()));
                return safeRefreshResult({
                  normalized,
                  signalsAccepted: journal.committedSignals,
                  persisted: true,
                  partial: normalized.partial || startupPruneFailed || !recoveredPrune.ok,
                  warnings: [
                    ...normalized.warnings,
                    ...(startupPruneFailed || !recoveredPrune.ok ? ['journal:prune_failed'] : []),
                  ],
                });
              }
            } else if (stored !== null
                && candidate?.refreshStartedAt === stored.refreshStartedAt
                && candidate.stateDigest === stored.stateDigest) {
              await abandonJournalStages(deps, exactNow(completionNow()), {
                mode: 'exact',
                generation: invocationNow.toISOString(),
                evidence: candidateAbandonmentEvidence('ready', candidate),
              });
            }
          } else if (afterFailure?.status === 'absent') {
            await abandonJournalStages(deps, exactNow(completionNow()), {
              mode: 'exact',
              generation: invocationNow.toISOString(),
              evidence: candidateAbandonmentEvidence('absent', stored),
            });
          }
        }
        return safeRefreshResult({
      ...
      return safeRefreshResult({
        ...
        signalsAccepted: journal.committedSignals,
        persisted: true,
        partial: normalized.partial || startupPruneFailed || !pruned.ok,
        warnings: [
          ...normalized.warnings,
          ...(startupPruneFailed || !pruned.ok ? ['journal:prune_failed'] : []),
        ],
      });
    });
  };
