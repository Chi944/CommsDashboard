...
const INSTITUTIONAL_ENTITY_IDS = Object.freeze({
...
function nowFrom(options) {
  const value = options?.now ?? options ?? new Date();
...
function assertProviderEntity(providerId, entityId) {
  const expected = INSTITUTIONAL_ENTITY_IDS[providerId];
  if (expected && entityId !== expected) throw schemaInvalid();
  ...
  if ((providerId.startsWith('hyperliquid-') && entityId.startsWith('polymarket:'))
      || (providerId.startsWith('polymarket-') && entityId.startsWith('hyperliquid:'))) throw schemaInvalid();
}
...
const ACTIVITY_FIELDS = Object.freeze([
  ...
  'disclosedAt', 'observedAt', 'retrievedAt', 'delaySeconds', 'summary', 'caveats', 'freshness',
]);

export function normalizeActivity(value, options = {}) {
  const now = nowFrom(options);
  assertSafePlainRecord(value, ACTIVITY_FIELDS);
  const providerId = safeIdentifier(value.providerId);
  const entityId = safeIdentifier(value.entityId);
  assertProviderEntity(providerId, entityId);
  let magnitude = null;
...
export function getProviderFreshnessPolicy(providerId) {
  ...
  if (providerId === 'sec-edgar' || providerId.startsWith('institutional-')
...
export function dedupeByStableId(records) {
  ...
  return records.map((record) => {
    if (!record || typeof record !== 'object') throw schemaInvalid();
...
function institutionalActivity(record, context) {
  ...
  return normalizeActivity({
    ...
    direction: null,
    magnitude: { value: record.reportedValueUsd, unit: 'reported_value_usd' },
    effectiveAt,
    disclosedAt,
    observedAt,
    retrievedAt: observedAt,
    delaySeconds: Math.max(0, (Date.parse(disclosedAt) - Date.parse(effectiveAt)) / 1_000),
    summary: `Official disclosure reports a BTC balance of ${record.btcAmount} for the stated period.`,
    caveats: ['A disclosed balance change is not evidence of a trade.'],
    freshness: 'fresh',
  }, context);
}
...
const SEC_FILING_FIELDS = Object.freeze([
  'cik', 'form', 'accessionNumber', 'periodEnd', 'filedAt', 'isAmendment', 'amendmentChain', 'primaryDocument',
]);

function secFilingActivity(filing, context) {
  assertSafePlainRecord(filing, SEC_FILING_FIELDS);
  ...
  const effectiveAt = canonicalTimestamp(`${filing.periodEnd}T00:00:00.000Z`, { now: nowFrom(context) });
  ...
  return normalizeActivity({
    ...
    delaySeconds: Math.max(0, (Date.parse(disclosedAt) - Date.parse(effectiveAt)) / 1_000),
    summary: `Official ${filing.form} filing for the period ending ${filing.periodEnd}.`,
    caveats: ['Filing data is delayed disclosure and is not live position data.'], freshness: 'fresh',
  }, context);
}
...
export function validateRankings(value, context = {}) {
  ...
  return {
    investors: dedupeByStableId(value.investors.map((entity) => normalizeEntity(entity, context))),
    crypto: {
      polymarket: {
        month: dedupeByStableId(value.crypto.polymarket.month.map((performance) => normalizePerformance(performance, context))),
      },
      hyperliquid: {
        month: dedupeByStableId(value.crypto.hyperliquid.month.map((performance) => normalizePerformance(performance, context))),
        allTime: dedupeByStableId(value.crypto.hyperliquid.allTime.map((performance) => normalizePerformance(performance, context))),
      },
    },
  };
}
...
export function validateAcceptedSnapshot(value, context = {}) {
  ...
  return {
    ...
    performances,
    signals,
    rankings: validateRankings(value.rankings, { now }),
