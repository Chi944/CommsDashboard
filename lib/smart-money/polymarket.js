...
export function normalizePolymarketLeaderboard(rows, context = {}) {
  ...
  return rows.flatMap((row) => {
    ...
    const volumeUsd = finiteNumber(row.vol);
...
export function joinPolymarketWindows(month, allTime, context = {}) {
  ...
  const add = (rows, window) => {
    for (const row of Array.isArray(rows) ? rows : []) {
      const address = row?.proxyWallet ?? row?.wallet;
      if (!isWallet(address)) continue;
      const sourceWindow = row?.windows?.[window];
      const pnlUsd = finiteNumber(sourceWindow?.pnlUsd ?? row.pnl);
      const volumeUsd = finiteNumber(sourceWindow?.volumeUsd ?? row.vol);
      const rank = finiteNumber(row.rank);
