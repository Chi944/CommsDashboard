...
function dashboardSummary(value) {
  return typeof value === 'string' ? value.trim().slice(0, MAX_DASHBOARD_SUMMARY_LENGTH) : '';
}

function cachePermitted(rightsId) {
  return ['fetch', 'cache', 'history', 'display', 'attribute']
...
export function normalizePublicationEntry(entry, config = {}) {
  ...
  const contentHash = config.contentHash || metadataHash({
  ...
  return {
    ...
    dashboardSummary: dashboardSummary(config.dashboardSummary),
  };
}
