...
export function assertSafeArray(value) {
  ...
  for (const [key, descriptor] of Object.entries(descriptors)) {
    ...
    if (!/^\d+$/.test(key) || !descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) throw schemaInvalid();
  }
  return value;
}
...
function normalizeEligibility(value, kind) {
  ...
  const reasons = kind === 'paper'
    ? ['supported_reference_price', 'unsupported_asset', 'research_only', 'missing_position_change', 'provider_research_only', 'missing_reference_price']
    : ['material_confirmed_change', 'material_change', 'new_evidence'];
  if (!reasons.includes(reason) || (kind === 'notification' && !value.eligible)) throw schemaInvalid();
  if (kind === 'paper' && value.eligible !== (reason === 'supported_reference_price')) throw schemaInvalid();
  return { eligible: value.eligible, reason };
}

function normalizeReferencePrice(value, asset, now) {
  ...
  return {
    ticker,
    price: finiteNumber(value.price, { nonnegative: true }),
    currency: stringValue(value.currency, { maxLength: 12 }).toUpperCase(),
    source: stringValue(value.source, { maxLength: 100 }),
    asOf: canonicalTimestamp(value.asOf, { now }),
...
function assertProviderEntity(providerId, entityId) {
  ...
  if (institutional[providerId] && institutional[providerId] !== entityId) throw schemaInvalid();
...
export function validateSignal(value, options = {}) {
  ...
  if (action === 'observe' && (positionChange !== null
      || !['research_only', 'unsupported_asset'].includes(paperEligibility.reason))) throw schemaInvalid();
  if (!asset.supported && paperEligibility.reason !== 'unsupported_asset') throw schemaInvalid();
  ...
  return {
    ...
    paperEligibility,
    referencePrice,
    freshness: enumValue(value.freshness, FRESHNESS_VALUES),
  };
}
