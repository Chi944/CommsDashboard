...
// Keep this reviewed runtime pin in sync with package.json. Dependency updates
// must pass the real, network-free PDF extraction test before changing it.
const PATCHED_PDFJS_VERSION = '6.3.289';
...
async function extractPdfPages(bytes) {
  let pdfjs;
  try {
    // Vercel traces only explicit imports. PDF.js otherwise reaches these
    // optional runtime files through createRequire/dynamic workerSrc and the
    // deployed function fails with missing DOMMatrix/pdf.worker.mjs.
    const canvas = await import('@napi-rs/canvas');
    if (typeof canvas?.DOMMatrix !== 'function'
        || typeof canvas?.ImageData !== 'function'
        || typeof canvas?.Path2D !== 'function') throw calendarError();
    globalThis.DOMMatrix ||= canvas.DOMMatrix;
    globalThis.ImageData ||= canvas.ImageData;
    globalThis.Path2D ||= canvas.Path2D;
    const worker = await import('pdfjs-dist/legacy/build/pdf.worker.mjs');
    if (typeof worker?.WorkerMessageHandler?.setup !== 'function') throw calendarError();
