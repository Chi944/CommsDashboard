...
async function loadProvider(provider, fetchText, from, through, configuredTimeoutMs) {
  try {
    ...
    // with FRED. The worst tier budget is 750 + max(750, 5,500) = 6,250ms,
    // leaving headroom beneath the browser's 8-second request deadline.
    ...
    // rejection handler now because that branch may return without awaiting FRED.
    fredAttempt.catch(() => {});
    try {
